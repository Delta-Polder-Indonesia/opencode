...
// Renderer session headers are applied per-request (or per-response) in
// `createMainWindow` since each window owns a session. These are the CORS
// response headers the `oc://` origin needs to reach the loopback sidecar.
...
/**
 * The loopback sidecar's CORS middleware already echoes the renderer origin
 * (in addition to `http://localhost:*`), so API calls from the packaged
 * `oc://` page are allowed without manual response fabrication. To stay
 * deterministic across Electron versions — and because Electron 42's
 * `onBeforeRequest` callback only supports `cancel`/`redirectURL`, not
 * response fabrication — we only stamp permissive response headers on this
 * window's session, mirroring the sidecar's `cors: ["oc://renderer"]`.
 */
function wireRendererSession(win: BrowserWindow) {
  ...
  session.webRequest.onHeadersReceived((details, callback) => {
    ...
    addResponseHeaders(responseHeaders, corsResponseHeaders)
    if (isRendererUrl(details.url, true)) {
      upsertKeyValue(responseHeaders, documentPolicyHeader, [jsCallStacksDocumentPolicy])
    }
...
function addResponseHeaders(headers: Record<string, any>, next: Record<string, string>) {
  for (const [key, value] of Object.entries(next)) {
    upsertKeyValue(headers, key, [value])
  }
}
