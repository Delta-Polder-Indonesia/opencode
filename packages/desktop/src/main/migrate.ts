...
function migrateFile(datPath: string, filename: string) {
  ...
  try {
    ...
    getLogger().warn("tauri migration: failed to parse", filename, err)
    return
  }
  ...
  getLogger().log("tauri migration: migrated", filename, "→", storeName, { migrated, skipped })
}

export function migrate() {
  if (getStore().get(TAURI_MIGRATED_KEY)) {
    getLogger().log("tauri migration: already done, skipping")
    return
  }

  const dir = tauriDir(tauriAppId())
  getLogger().log("tauri migration: starting", { dir })

  if (!existsSync(dir)) {
    getLogger().log("tauri migration: no tauri data directory found, nothing to migrate")
  ...
  getLogger().log("tauri migration: complete")
  getStore().set(TAURI_MIGRATED_KEY, true)
}
