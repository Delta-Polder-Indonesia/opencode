...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectOpencodeVersion(APP_VERSION, APP_VERSION)).not.toThrow()
  expect(() => expectOpencodeVersion("1.14.35", APP_VERSION)).toThrow(
    `OpenCode update finished but Debian still reports 1.14.35; expected ${APP_VERSION}`,
  )
})
...
test("probes addable distros in parallel before checking OpenCode", async () => {
  ...
  const controller = createWslServersController(APP_VERSION, async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
...
function testControllerOptions() {
  return {
    ...
    readCommandVersion: async () => APP_VERSION,
